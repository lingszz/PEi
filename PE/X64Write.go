...
func (x X64) WriteSecurityDirectorySizeAndPE(ShellBytesLen uint32) []byte {
	...
	for i := 0; i < 4; i++ {
		...
		x.FileBytes[uint32(0xAC)+x.AddressOfNewExeHeader+uint32(i)] = WeNeedSize[i]
	}
	return x.FileBytes
}

func (x X64) WriteCheckSum() []byte {
...
func (x X64) ShellcodeSteganography(ShellBytes []byte) []byte {
